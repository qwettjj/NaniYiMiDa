...
public interface RecipeRepository extends JpaRepository<Recipe, Long> {
        ...
        @org.springframework.data.jpa.repository.Query("SELECT r FROM Recipe r WHERE r.status = :status " +
                        "AND (:startDate IS NULL OR r.createTime > :startDate) " +
                        "AND (LOWER(r.title) LIKE LOWER(CONCAT('%', :keyword, '%')) " +
                        "OR LOWER(r.description) LIKE LOWER(CONCAT('%', :keyword, '%')))")
        Page<Recipe> searchByStatusAndStartDateAndKeyword(
                        @org.springframework.data.repository.query.Param("status") RecipeStatus status,
                        @org.springframework.data.repository.query.Param("startDate") Date startDate,
                        @org.springframework.data.repository.query.Param("keyword") String keyword,
                        Pageable pageable);

        @org.springframework.data.jpa.repository.Query("SELECT r FROM Recipe r JOIN r.ingredients i WHERE r.status = :status "
                        +
                        "AND (:startDate IS NULL OR r.createTime > :startDate) " +
                        "AND i.ingredient = :ingredient")
        Page<Recipe> searchByStatusAndStartDateAndIngredient(
                        @org.springframework.data.repository.query.Param("status") RecipeStatus status,
                        @org.springframework.data.repository.query.Param("startDate") Date startDate,
        ...
        @org.springframework.data.jpa.repository.Query("SELECT r FROM Recipe r JOIN r.ingredients i WHERE r.status = :status "
                        ...
                        "OR LOWER(r.description) LIKE LOWER(CONCAT('%', :keyword, '%'))) " +
                        "AND i.ingredient = :ingredient")
        Page<Recipe> searchByStatusAndStartDateAndKeywordAndIngredient(
                        @org.springframework.data.repository.query.Param("status") RecipeStatus status,
                        @org.springframework.data.repository.query.Param("startDate") Date startDate,
                        @org.springframework.data.repository.query.Param("keyword") String keyword,
                        @org.springframework.data.repository.query.Param("ingredient") IngredientEnum ingredient,
